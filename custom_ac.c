...
#define YES 1
#define NO 0
#define timeON 2000	// milliseconds
#define timeOFF 2000	//milliseconds
#define timeON2min 120000 //milisecond
...
void seqA_AC (int test_cycle, status *testStatus)
{
	for (int n=1; n<=test_cycle; n++)
    {
		...
		printf("\rRound 0: \t\t\tON = 10s \tState: ON  \t");
		fflush(stdout);
		...
		{
			//digitalWrite(relayPin, HIGH); 	// Turn ON
			//printf("\rRound %d: \tOFF = %dms \tON = %ds \tState: ON  \t", i, timeOFF, timeON/1000);
			//fflush(stdout);
			...
			printf("\rRound %d: \tOFF = %dms \tON = %ds \tState: OFF \t", i, timeOFF, timeON/1000);
			fflush(stdout);
			...
			digitalWrite(relayPin, HIGH); 	// Turn ON
			printf("\rRound %d: \tOFF = %dms \tON = %ds \tState: ON \t", i, timeOFF, timeON2min/1000);
			fflush(stdout);
			wait_ms(timeON2min, testStatus);
			//delay(i*timeOFF_step); 			// Wait 
			if (testStatus->state == STOP)
				return;	
		}
