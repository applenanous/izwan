...
#define YES 1
#define NO 0
#define RCOFF 30000	// milliseconds
#define timeOFF 10000	//milliseconds
#define timeON 60000 //milisecond
//#define timeOFF2 5000	//milliseconds
//#define timeON2 300000 //milisecond
#define RCON 60000	// milliseconds
...
void seqA_AC (int test_cycle, status *testStatus)
{
	for (int n=1; n<=test_cycle; n++)
    {
		...
		printf("\rRound 0: \t\t\tACON = 60s \tState: ON  \t");
		fflush(stdout);
		...
		for (int i=1; i<=100; i++)	
		{
		   ...
		   printf("\rRound %d: \tRCOFF = 30s \tState: RC OFF  \t");
		   fflush(stdout);
		   wait_ms(RCOFF, testStatus);
		   //delay(60000);
		   if (testStatus->state == STOP)
			return;
			
			digitalWrite(relayPin, LOW); 	// Turn OFF
			printf("\rRound %d: \tACOFF = %dms \tState: AC OFF \t", i, timeOFF, timeON/1000);
			fflush(stdout);
			...
			digitalWrite(relayPin, HIGH); 	// Turn ON
			printf("\rRound %d: \tACON = %ds \tState: AC ON  \t", i, timeOFF, timeON/1000);
			fflush(stdout);
		   ...
		   sendIR_cmd(IRPin,POWER_ON);  //RC ON
		   printf("\rRound %d: \tRCON = 60s \tState: RC ON  \t");
		   fflush(stdout);
		   wait_ms(RCON, testStatus);
		   //delay(60000);
		   if (testStatus->state == STOP)
			return;
		}
